    ...
    def init(self):
        ...
        if self.auto_start_worker:
            if self.hass and hasattr(self.hass, 'async_create_task'):
                ...
                self.hass.async_create_task(self.device_Worker())
            else:
                # Standard asyncio Umgebung
                asyncio.create_task(self.device_Worker())
            
            _LOGGER.info("Device worker started automatically")
        else:
        ...
        try:
            ...
            async with asyncio.timeout(60):  # 60 Sekunden Timeout
                ...
                groupedRoomEntities = await asyncio.wait_for(
                    self.regListener.get_filtered_entities_with_value(self.room.lower()),
                    timeout=30.0
                )
